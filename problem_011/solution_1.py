...
Matrix = [list(map(int, line.split())) for line in grid.splitlines()]
max_product = 0
grid_size = 20
adj_size = 4
...
for i in range(grid_size-adj_size+1):
    for j in range(grid_size):
        ...
        for a in range(adj_size):
            ...
            if j >= adj_size-1:
                product_dl *= Matrix[i+a][j-a]
        max_product = max(max_product,product_x,product_y,product_dr,product_dl)
print(max_product)
